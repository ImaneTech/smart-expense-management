...
// Assurez-vous que le chemin vers votre config est correct.
require_once __DIR__ . '/../../config.php';
// Inclusion du contrôleur qui gère la logique d'historique (getHistorique)
require_once BASE_PATH . 'controllers/DemandeController.php'; 
require_once BASE_PATH . 'includes/header.php';

// --- INITIALISATION DU CONTRÔLEUR ---

// 1. Assurez-vous que la connexion PDO est disponible (définie dans config.php)
if (!isset($pdo)) {
    die("Erreur: La connexion à la base de données (\$pdo) est manquante.");
}

// 2. Instancier le contrôleur. Cela vérifie l'authentification et récupère le managerId.
                            ...
                            <?php foreach ($historique as $h): ?>
                                <tr> 
                                    ...
                                    <td>
                                        ...
                                        <small class="text-muted"><?= $h['date_depart'] ? date('d/m/Y', strtotime($h['date_depart'])) : 'N/A' ?></small>
                                    </td>
                                    <td>
                                        <?php if(isset($h['date_validation']) && $h['date_validation']): ?>
                                    ...
                                    <td class="fw-bold text-primary">
                                        <?= number_format($h['total_calcule'] ?? 0, 2) ?> €
                                    </td>
                                    <td>
                                        <?php 
                                            $badgeClass = 'bg-secondary';
                                            $statut = $h['statut'] ?? 'Inconnu';
                                            // Utilisation de strpos pour une correspondance partielle (ex: 'Validée Manager' ou 'Validée Compta')
                                            if (strpos($statut, 'Validée') !== false || $statut == 'Approuvée Compta') {
                                    ...
                                    <td class="text-end pe-4">
                                        <a href="details_demande.php?id=<?= (int)$h['id'] ?>" class="btn btn-sm btn-outline-primary" title="Revoir le dossier">
